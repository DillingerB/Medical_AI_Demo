1st: install node.js if you have not already, this will make the process easier.
...
OUR DEMO KEY FEATURES: both a provider and patient side, patient and provider speak to one another, our AI gives a notification warning to user, and provider (for specific user)[bug: look under provider for correct alert feature, design flaw for alert]
Highlight AI module, is quite tiring to make, and is just a "demo" style AI, has no brain, just knows bad medical combos

FEATURES TO ADD: toxic dosage amount (updates database, server, prescription. Tells what the lethal dosage is in a substance, alerts patient/provider about to OD)

can present with my laptop incase things go wrong, can talk about code, can descrine demo and what happens. The code SHOULD WORK on my device as I HAVE TESTED it, both happy and sad, so happy to share. Thanks for baring with me being slightly stupidly coding.
